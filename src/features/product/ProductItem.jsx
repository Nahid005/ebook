...
function ProductItem({product}) {
    const {title, image, price, ratings, author, id} = product;

    return (
        <div className="flex flex-col gap-2 group">
            <div className="relative overflow-hidden">
                <Link to={`product/${id}`}>
                    <img className="rounded-lg transition-transform duration-300 group-hover:scale-105" src={image} alt={title} />
                </Link>
            ...
            <Link to={`product/${id}`}>
                <div className="flex flex-col">
                    <h5 className="font-bold text-base text-neutral-700">{title}</h5>
                    <p className="font-medium text-sm text-neutral-700">{author}</p>
